...
bool CactusWrench::onEnable() {
    try {
        ...
        mItemUseListener = bus.emplaceListener<PlayerInteractBlockEvent>([this](PlayerInteractBlockEvent& event) {
...
bool CactusWrench::isRotatableBlock(const Block& block) {
    // 检查方块是否为可旋转的红石元件
    const std::string& blockName = block.getTypeName();

    // 支持的可旋转红石元件
    return blockName.find("observer") != std::string::npos || blockName.find("piston") != std::string::npos
        || blockName.find("hopper") != std::string::npos || blockName.find("dispenser") != std::string::npos
        || blockName.find("dropper") != std::string::npos || blockName.find("repeater") != std::string::npos
        || blockName.find("comparator") != std::string::npos || blockName == "minecraft:observer"
        || blockName == "minecraft:piston" || blockName == "minecraft:sticky_piston" || blockName == "minecraft:hopper"
        || blockName == "minecraft:dispenser" || blockName == "minecraft:dropper" || blockName == "minecraft:repeater"
...
void CactusWrench::rotateBlock(Player* player, const BlockPos& pos, const Block& block) {
    try {
        ...
        // 尝试旋转方块
        performRotation(blockSource, pos, currentBlock);
...
bool CactusWrench::performRotation(BlockSource& blockSource, const BlockPos& pos, const Block& block) {
    try {
        ...
        // 根据方块类型决定旋转规则
        if (typeName == "observer" || typeName == "piston" || typeName == "sticky_piston" || typeName == "dispenser"
            || typeName == "dropper") {
            // 6方向旋转
            ...
            canRotate = true;
        } else if (typeName.find("stairs") != std::string::npos || typeName.find("trapdoor") != std::string::npos) {
            ...
            canRotate     = true;
                   ...
                   || typeName == "comparator" || typeName == "unpowered_comparator"
        ...
        } else if (typeName == "furnace" || typeName == "lit_furnace" || typeName == "blast_furnace"
                   || typeName == "lit_blast_furnace" || typeName == "smoker" || typeName == "lit_smoker") {
            // 熔炉类: 2-5为朝向
            if (variant >= 2 && variant <= 5) {
                newVariant = ((variant - 2 + 1) % 4) + 2;
                canRotate  = true;
