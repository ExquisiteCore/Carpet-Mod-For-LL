...
ProfilerModule::ProfilerModule() : BaseModule("Profiler", "carpet.module.profiler.description"_tr()) {}
...
void ProfilerModule::profileNormal(int ticks) { startProfiling(ticks, "normal"); }

void ProfilerModule::profileChunk(int ticks) { startProfiling(ticks, "chunk"); }

void ProfilerModule::profileEntity(int ticks) { startProfiling(ticks, "entity"); }

void ProfilerModule::profilePendingTick(int ticks) { startProfiling(ticks, "pt"); }

void ProfilerModule::profileMSPT(int ticks) { startProfiling(ticks, "mspt"); }
...
void ProfilerModule::printEntityResults() {
    ...
    for (size_t i = 0; i < std::min(size_t(10), sortedEntities.size()); i++) {
        ...
        mod->getLogger()
            .info("  {}: {:.2f}ms total, {:.3f}ms avg ({})", type, totalTime, data.getAverageMs(), data.sampleCount);
    }
