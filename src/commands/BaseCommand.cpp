...
bool CommandContext::isConsole() const { return origin->getOriginType() == CommandOriginType::DedicatedServer; }
...
void CommandContext::success(const std::string& message) const { output->success(message); }

void CommandContext::error(const std::string& message) const { output->error(message); }
...
void BaseCommand::addSubCommand(const SubCommandInfo& subCmd) { subCommands.push_back(subCmd); }
...
bool BaseCommand::registerCommand() {
    try {
        ...
        for (const auto& subCmd : subCommands) {
            command.overload()
                .text(subCmd.name)
                .execute([this, subCmdName = subCmd.name](CommandOrigin const& origin, CommandOutput& output) {
...
void BaseCommand::execute(const CommandContext& ctx) {
    ...
    try {
        ...
        auto mod = ll::mod::NativeMod::current();
        mod->getLogger().error("Exception in command '{}' subcommand '{}': {}", commandName, subCmd->name, e.what());
    }
}
...
const std::vector<std::unique_ptr<BaseCommand>>& CommandManager::getAllCommands() { return commands; }

void CommandManager::cleanup() { commands.clear(); }
